"""
这段代码实现了Reeds-Shepp路径的计算和生成。
Reeds-Shepp路径是一种用于机器人和自动驾驶汽车路径规划的算法，
能够生成允许车辆前进和后退的路径。
代码的核心部分包括路径类型的生成、局部路径的插值以及各种路径组合的计算。

要生成Reeds-Shepp路径，应调用 calc_optimal_path 函数。此函数计算从起始点到目标点的所有可能路径，并选择最短的路径。
输入参数
    sx: 起点的 x 坐标
    sy: 起点的 y 坐标
    syaw: 起点的航向角（弧度）
    ...
    gyaw: 终点的航向角（弧度）
    maxc: 最大曲率（最大转弯半径的倒数）
    step_size（可选）: 步长（默认值为 STEP_SIZE）
...
"""
PATH 类用于存储路径的各个部分，包括长度、类型、总长度、位置、航向角和方向。
"""
...
"""
这个函数计算从起始点到目标点的所有可能路径，并选择最短的路径。
"""
...
"""
这个函数生成从起点到终点的所有路径，并将路径坐标从局部坐标系转换到全局坐标系。
"""
...
# 这些函数计算不同类型的路径（如LSL、LSR、LRL等），并调用set_path函数来保存它们。
def LSL(x, y, phi):
...
def main():
    ...
    for i in range(len(path_x)):
        ...
        for x, y, theta in states:
            draw_curve.Arrow(x, y, np.deg2rad(theta), 2, 'blueviolet')

        draw_curve.Car(path_x[i], path_y[i], yaw[i], 1.5, 3)
